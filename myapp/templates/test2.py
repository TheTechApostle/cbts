...
def add_questions(request, exam_id=None):
    """
    Allows teachers to add questions to an exam.
    Step 1: Select Department → Class → Course
    Step 2: Add questions for that course’s exam.
    """
    teacher = getattr(request.user, 'teacher_profile', None)
    if not teacher:
        messages.error(request, "You must be logged in as a teacher.")
    ...
    if request.method == 'POST' and 'save_questions' in request.POST:
        ...
        while True:
            ...
            if not q_text or not q_text.strip():
                ...
                if f'question_text_{question_index + 1}' not in request.POST:
                    break
                question_index += 1
                continue
